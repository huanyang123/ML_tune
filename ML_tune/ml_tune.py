'''
======================================================================================
|  A general utility module to auto-select the best model and auto-tune parameters   |
...
2. Tune a particular model : 
2a. For classification,  tune.tune_classifier('XGBClassifier',X_train, y_train, X_test, y_test)
2b. For regression,      tune.tune_regressor('XGBregressor',X_train, y_train, X_test, y_test)

3. Tune all the models : 
3a. For classification,  tune.tune_classifier_all(X_train, y_train, X_test, y_test)
3b. For regression,      tune.tune_regressor_all(X_train, y_train, X_test, y_test)

4. Get help and see the usage of all the functions:
   type "help(ut)"
...
from sklearn.metrics import accuracy_score, f1_score   #for classifier
...
#    classifier=['KNeighborsClassifier','ExtraTreesClassifier']
    ...
    best=sorted(all_result, key=lambda x: x[3], reverse=True)
    columns=['model_inp', 'train_accu','test_accu','f1_score','cpu','para','model']
    df_results = pd.DataFrame(best, columns=columns)
  
    print('==================================================================')
    print('\nThe table for each model\n', df_results.iloc[:,0:5], '\n')
    print('\nThe best model=', df_results.iloc[0,6])
...
def tune_classifier(model, X_train, y_train, X_test, y_test, tune=1):
    '''A general function to auto-tune hyperparameters for list of ML algorithms.
    
    Parameters as below:
    X_train, y_train:  data (np or df format) for tuning. y_train is the target.
    X_test, y_test:  data (np or df format) for validation only. y_test is the target.
    tune:   a key to indicate tune (=1) or not tune (=0) the hyper-parameters.    
    '''
    
    print('\n',f'----------------Tuning hyperparameters: {model}-------------------------','\n')           
    ...
    if model=='RandomForestClassifier':
       ...
       h_para=dict(max_depth=[2,3,5,7, 9],subsample=[0.7, 1],n_estimators=[300],
                  colsample_bytree=[0.7 ,1], num_leaves=[4, 8, 10, 14]
                  )
       mod = LGBMClassifier( )
    ...
    mod.fit(X_train, y_train)  #fit the model 
...
#    auc = roc_auc_score(y_test, prob)
    ...
    print('\nBest  train_score={:.3f},  test_score={:.3f},  f1_score={:.3f}, \
    CPU time= {:.2f} s'.format(train_score, test_score, f1_macro, time_dif))
        
#    print(f'Best train_score={train_score}: test_score={train_score} : f1_score={f1_test} \
#    : CPU time= {time_dif}')
    print('best_params=', grid.best_params_)
    print('model=', grid.best_estimator_)
    
    result=[model, train_score, test_score, f1_macro, time_dif, 
